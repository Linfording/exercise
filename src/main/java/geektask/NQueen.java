...
public class NQueen {
    ...
    public static void main(String[] args) {
        ...
        final List<List<String>> lists = solution.solveNQueens(5);
        System.out.println(JsonUtils.toJsonString(lists));
        final List<List<String>> lists2 = solution.solveNQueens(4);
        System.out.println(JsonUtils.toJsonString(lists2));
        final List<List<String>> lists3 = solution.solveNQueens(2);
        System.out.println(JsonUtils.toJsonString(lists3));
    }
    ...
    class Solution {
        ...
        private void solveChess(List<List<String>> result, char[][] chessBoard, int row) {
            if (row == chessBoard.length) {
                result.add(printChess(chessBoard));
                return;
            }
...
//                1. 列、斜 冲不冲突
        ...
        private List<String> printChess(char[][] chessBoard) {
            int n = chessBoard.length;
            List<String> chessBoardOne = new ArrayList<>();
            for (int i = 0; i < chessBoard.length; i++) {
                chessBoardOne.add(new String(chessBoard[i]));
            }
            return chessBoardOne;
        }
    }
